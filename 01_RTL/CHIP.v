...
wire [31:0] hit_word = (word_idx == 2'd0) ? data[set_idx][31:0] :
                       (word_idx == 2'd1) ? data[set_idx][63:32] :
                       (word_idx == 2'd2) ? data[set_idx][95:64] :
                                            data[set_idx][127:96];
wire [31:0] refill_word = (word_idx_r == 2'd0) ? mem_rdata[31:0] :
                          (word_idx_r == 2'd1) ? mem_rdata[63:32] :
                          (word_idx_r == 2'd2) ? mem_rdata[95:64] :
                                                 mem_rdata[127:96];
integer si;
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        case (state)
            S_IDLE: begin
                if (flush && !flush_done_r) begin
                    ...
                    if (hit) begin
                        if (wen) begin
                            case (word_idx)
                                2'd0: data[set_idx][31:0] <= wdata;
                                2'd1: data[set_idx][63:32] <= wdata;
                                2'd2: data[set_idx][95:64] <= wdata;
                                2'd3: data[set_idx][127:96] <= wdata;
                            endcase
                            dirty[set_idx] <= 1'b1;
                        end
            ...
            S_REFILL: begin
                if (mem_ready) begin
                    data[set_idx_r][31:0] <= (wen_r && word_idx_r == 2'd0) ? wdata_r : mem_rdata[31:0];
                    data[set_idx_r][63:32] <= (wen_r && word_idx_r == 2'd1) ? wdata_r : mem_rdata[63:32];
                    data[set_idx_r][95:64] <= (wen_r && word_idx_r == 2'd2) ? wdata_r : mem_rdata[95:64];
                    data[set_idx_r][127:96] <= (wen_r && word_idx_r == 2'd3) ? wdata_r : mem_rdata[127:96];
                    tag[set_idx_r] <= tag_addr_r;
                    valid[set_idx_r] <= 1'b1;
