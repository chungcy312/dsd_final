...
\subsection{Final report requirement}
The project description explicitly requires the June 14 submission folder to
contain the final presentation slides and a detailed report of 8--16 pages.
It does not prescribe a chapter template.  This report therefore emphasizes
architecture, implementation decisions, experimental evidence, synthesis
...
\resizebox{\linewidth}{!}{\begin{tikzpicture}[
  ...
  \draw[arr,red] (ex.north) -- (redirectTop)
    -| node[pos=0.28,above,font=\scriptsize]{redirect} (if.north);
...
\begin{tabular}{llrrr}
\toprule
Pattern & Cache & 16 blocks, 1-way & 32 blocks, 1-way & 64 blocks, 2-way\\
\midrule
QSort & I miss rate & 2.5498\% & 0.0541\% & 0.0541\%\\
QSort & D miss rate & 6.7210\% & 3.8209\% & 0.9843\%\\
Conv & I miss rate & 3.6880\% & 0.9164\% & 0.8941\%\\
Conv & D miss rate & 21.2606\% & 2.9163\% & 1.3641\%\\
LFSR\_HIST & I miss rate & 18.4577\% & 0.0130\% & 0.0130\%\\
LFSR\_HIST & D miss rate & 37.5730\% & 36.5347\% & 34.7826\%\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Final 2-way D-cache synthesis summary.}
\label{tab:syn}
\begin{tabular}{lr}
\toprule
Metric & Result\\
...
Estimated total dynamic power & \SI{99.6629}{mW}\\
Estimated leakage power & \SI{147.5071}{\micro W}\\
\bottomrule
\end{tabular}
\end{table}
...
For comparison, a synthesized one-way D-cache candidate at \SI{2.71}{ns} had a
similar cell area (\SI{767791.73}{\micro m^2}) but a deeper 20-level critical
path through the D-cache write path.  The two-way implementation's modest area
increase was accepted for its miss-rate benefit and shorter reported critical
logic depth.

\textbf{Report caveat.} The final QoR has no setup or hold violations, but the
constraint report contains one small maximum-capacitance violation
(\(-0.03\) in the library's capacitance units) on a register-file net.  The
automatically reported \texttt{max\_area} and \texttt{max\_leakage\_power}
entries are artifacts of setting their optimization target to zero, rather
than functional or timing failures.  The power values are vectorless
estimates, so they are useful for comparison but not workload-accurate power.
...
Future work should first remove the remaining maximum-capacitance violation and
add activity-annotated power analysis.  Architectural extensions should be
evaluated in this order: a small nonblocking or write-buffered D-cache, a
...
\section*{References}
\begin{enumerate}
  \item Digital System Design Final Project, \emph{Pipelined RISC-V Design},
        project description and submission requirements, 2026.
  \item RISC-V International, \emph{The RISC-V Instruction Set Manual,
        Volume I: Unprivileged ISA}.
  \item Synopsys Design Compiler synthesis, timing, area, constraint, and power
        reports generated for the final Group 1 implementation.
  \item Group 1 cache and branch-predictor sweep records and final presentation.
\end{enumerate}
